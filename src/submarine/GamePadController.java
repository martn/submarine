...
public class GamePadController {
    ...
    public GamePadController() throws GamepadNotFoundException {
        ...
        if (cs.length == 0) {
            throw new GamepadNotFoundException("NO CONTROLERS FOUND");
        }

        // print the name and type for each gpController
    ...
    public float[] getXYStickValue() {
        return getCompassValue(xAxisIdx, yAxisIdx);
    }
    
    public float[] getZRZStickValue() {
        return getCompassValue(zAxisIdx, rzAxisIdx);
    }
    
    private float[] getCompassValue(int xA, int yA) {
        float ret[] = new float[2];
        ret[0] = comps[xA].getPollData();
        ret[1] = comps[yA].getPollData();        
        return ret;
    }
